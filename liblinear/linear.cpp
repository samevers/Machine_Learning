...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	for(i=0;i<nr_w;i++)
		dec_values[i] = 0; // predict outcome
	for(; (idx=lx->index)!=-1; lx++)
	{
		...
		if(idx<=n)
			for(i=0;i<nr_w;i++) 
			{
				// svm's predict score is : weight[] * x[]
				dec_values[i] += w[(idx-1)*nr_w+i]*lx->value;// i.e., weight of each position MULTIPLY value of features weight.
			}
	}
	...
	if(nr_class==2)
	{
		if(check_regression_model(model_))
			...
			//return dec_values[0];
			//return (dec_values[0]>0)?dec_values[0]:0; // by yonghua.
...
double predict_probability(const struct model *model_, const struct feature_node *x, double* prob_estimates)
{
	if(check_probability_model(model_))
	{
		...
		double label=predict_values(model_, x, prob_estimates); // predict_values
		for(i=0;i<nr_w;i++)
