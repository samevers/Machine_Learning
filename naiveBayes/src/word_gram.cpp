...
const static string CLASSNAME[] = {"video","image","music","shop","job","novel","soft","wenda","web","fangchan"};
		...
		for(iter_y = hash_y_prob.begin(); iter_y != hash_y_prob.end(); iter_y++)
		{
			...
			for(int i = 1; i < vec_tmp.size(); i++)
			{
				...
				if(i < vec_tmp.size() - 2)
				{
					...
					{
						prob_mul *= hash_model[make_pair(tri_bgram,y)];
						prob_add += hash_model[make_pair(tri_bgram,y)];
